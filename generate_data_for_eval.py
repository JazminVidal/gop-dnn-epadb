...
# * textgrid_list: a list of all possible TextGrid files for Epa-DB.
...
import numpy as np
import textgrids # pip install git+https://github.com/kylerbrown/textgrid
...
def generate_dict_from_transcripctions(transcriptions):
    ...
    for line in open(transcriptions,'r'):
        ...
        if sent not in trans_dict_clean:
            ...
            orig_trans_dict[sent] = list()


        orig_trans_dict[sent].append(fields[2:])
    ...
    for sent, v in trans_dict.items():
        trans_clean_new = []
        for i in range(0, len(v)):
            trans_clean_new.append(clean_zeros(v[i]))
    ...
    for line in open(transcriptions,'r'):
        ...
        for i in range(2, len(fields)):
            ...
            trans_clean_new = []
            for p in phones:
                for t in trans_clean:
                    if p != '0':
                        t_tmp_clean = t + [p]
                        trans_clean_new.append(t_tmp_clean)
            trans_clean = trans_clean_new

        trans_dict_clean[sent] += trans_clean


    return trans_dict, trans_dict_clean, orig_trans_dict, sent_dict
...
def generate_data_for_eval(transcriptions, transcriptions_complete, textgrid_list, gop_path, phones_pure_path, labels_dir, log_dir, export_path):
    ...
    trans_dict, trans_dict_clean, orig_trans_dict, sent_dict = generate_dict_from_transcripctions(transcriptions)
    trans_dict_complete, trans_dict_clean_complete, orig_trans_dict_complete, sent_dict_complete = generate_dict_from_transcripctions(transcriptions_complete)
    ...
    for waveform in [l.strip() for l in open(textgrid_list, 'r').readlines()]:

        spk = waveform.split("_")[0]
        sent = waveform.split("_")[1]
        ...
        tgfile = "%s/%s/%s.TextGrid"%(labels_dir, spk, waveform)
        ...
        tgfile = "%s/%s/%s_%s.TextGrid"%(labels_dir, spk, spk, sent)
        print("Sentence %s: %s (File: %s)"%(sent," ".join(sent_dict[sent]), tgfile))
    ...
    joblib.dump(df_trans_match, 'data_for_eval.pickle')
...
def main(args):
    generate_data_for_eval(args.trans_SAE_path, args.trans_complete_path, args.textgrids_list, args.gop_path, args.phones_pure_path, args.labels_dir, args.log_dir, args.export_path)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--labels-dir', dest='labels_dir', help='Directory with manual textgrid files', default=None)
