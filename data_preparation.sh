...
[ -d exp ] || tar -zxvf 0013_librispeech_s5.tar.gz

# Move folders to the corresponding directories
cp -r 0013_librispeech_v1/exp/nnet3_cleaned/chain_cleaned/tdnn_1d_sp exp/nnet3_cleaned
...
# Extract ivectors
nspk=$(wc -l <data/epa_test_hires/spk2utt)
steps/online/nnet2/extract_ivectors_online.sh --cmd "run.pl" --nj "${nspk}" data/test_data_hires exp/nnet3_cleaned/extractor exp/nnet3_cleaned/ivectors_test_epa_hires
...
mkdir exp
mkdi data
cp -r data/test_epa_hires data/
mv test_epa_hires test_epa
