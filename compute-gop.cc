...
// start normalization
      if (norm) {
        Vector<BaseFloat> norm_factor(probs.NumRows());
        norm_factor.AddColSumMat(1.0, probs, 0.0);
        norm_factor.InvertElements();
        probs.MulRowsVec(norm_factor);
      }
// end normalization
      ...
      for (int32 i = 0; i < frame_num; i++) {
        ...
        if (next_phone_id != cur_phone_id) {
          ...
          BaseFloat gop;
          if (max == true){
            gop = lpp_part(cur_phone_id) - lpp_part.Max();
          } else {
            gop = lpp_part(cur_phone_id);
          }
